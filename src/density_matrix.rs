...
impl DensityMatrix {
    ...
    pub fn expectation_single(&self, op: OneQubitOp, index: usize) -> Result<Complex<f64>, &str> {
        ...
        let mut result_tensor = self
            .tensor
            .tensordot(&op_tensor.tensor, (&[1], &[index]))
            .unwrap();
    ...
    pub fn evolve_single(&mut self, op: &Operator, index: usize) -> Result<(), String> {
        ...
        self.tensor = self
            .tensor
            .tensordot(
                &Tensor::from_vec(op.transconj().tensor.data, vec![2, 2]),
        ...
        self.tensor = self
            .tensor
            .moveaxis(
                &[0, (self.tensor.shape.len() - 1).try_into().unwrap()],
                &[
    ...
    pub fn evolve(&mut self, op: &Operator, indices: &[usize]) -> Result<(), String> {
        ...
        self.tensor = op
            .tensor
            .tensordot(&self.tensor, (&first_axe, &second_axe))
            .unwrap();
        ...
        self.tensor = self
            .tensor
            .tensordot(&op_transconj.tensor, (&first_axe, &second_axe))
            .unwrap();
    ...
    pub fn equals(&self, other: DensityMatrix, tol: f64) -> bool {
        if self.tensor.shape.iter().product::<usize>()
            == other.tensor.shape.iter().product::<usize>()
        {
    ...
    pub fn ptrace(&mut self, qargs: &[usize]) -> Result<(), &str> {
        ...
        let rho_res = id_tensor
            .tensordot(&self.tensor, (&tensordot_first_axe, &trace_axes))
            .unwrap();
        self.tensor = rho_res;
